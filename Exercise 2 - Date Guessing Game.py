...
def guess_game(first = calendar(month_names, num_days_in_month)):
   ...
   if val == 1:
       return guess_game(first[:mid + 1]) # If the date is earlier, will get all the dates that are before the index of the guessed date in list and run guess_game again with list split in half
   elif val == 2:
       return guess_game(first[mid - 1:]) # If the date is later, will get all the dates that are before the index of the guessed date in list and runs guess_game again with list split in half
   else:
