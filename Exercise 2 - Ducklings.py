...
# Loop to continue creating ducklings in text space
    while True:  
        for laneNum, ducklingObj in enumerate(ducklingLanes): # Goes through ducklingLanes and gets the laneNum (index) and the ducklingObj (element)
            ...
            if (ducklingObj == None and random.random() <= DENSITY): # If element is None and a random value is less than or equal to DENSITY
                    ...
                    ducklingObj = Duckling()                       # Create new Duckling object and assign it to current element
            ...
            if ducklingObj != None: # If ducklingObj is not equal to none
                ...
                print(' ' * DUCKLING_WIDTH, end='') # If the ducklingObj is none and random value is more then DENSITY, finish drawing the current duck
...
class Duckling:
    ...
    def __init__(self):
        # Assigns a random attribute to duckling
    ...
    # Creates head duckling as string based on instance variables and returns string to display
    def getHeadStr(self):
        headStr = ''
        ...
        return headStr
    # Creates body of duckling as string based on instance variables and returns string to display
    def getBodyStr(self):
        bodyStr = '('  
        if self.direction == LEFT:
            if self.body == CHUBBY:
        ...
        return bodyStr
    # Creates feet of duckling as string based on instance variables and returns it to display
    def getFeetStr(self):
        if self.body == CHUBBY:
            return ' ^^  '
        elif self.body == VERY_CHUBBY:
    ...
    def getNextBodyPart(self):
        if self.partToDisplayNext == HEAD: # If going to display HEAD already, display BODY next
